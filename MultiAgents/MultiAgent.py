...
from Agents.l2rpn_base_agent import L2rpnAgent, SingleAgent
...
class IMARL(L2rpnAgent):
    ...
    def agent_act(self, obs, is_safe, sample, dn_count=0) -> BaseAction:
        ...
        if not is_safe or (len(self.sub_picker.subs_2_act) > 0):
            # print()
            # print("subs to act: ",self.sub_picker.subs_2_act)
            with torch.no_grad():
...
class ReArIMARL(L2rpnAgent):
    ...
    def save_start_transition(self):
        super().save_start_transition()
        re_ar, action = self.start_goal
        self.agents[re_ar].save_start_transition()
    ...
    def save_transition(self, reward, done, n_step=1):
        ...
        next_adj = self.adj.clone()
        re_ar, action = self.start_goal
        self.agents[re_ar].save_transition(
            self.start_state,
            self.start_adj,
            action,
            reward,
    ...
    def load_model(self, path, name=None):
        [agent.load_model(f"{path}", f"{name}_ra_{sub}") for sub, agent in self.agents.items()]
